...
|---|---|---|---|
| Wi-Fi / SSH / VNC / Web | `hostname -I`, SSH login, VNC login, `/health` | LOCAL_NOT_RUN | Network access must stay available after disabling the official App. |
...
| Audio cues | `POST /api/audio/announce` or local playback helper | LOCAL_NOT_RUN | Shelf cue, item cue, and missing-item voice alarm should be distinguishable. |
...
- Black tape is not the main line-following path. Four-channel full black is the shelf-row end trigger; unexpected partial tape contact is a safety/forbidden-zone signal.
