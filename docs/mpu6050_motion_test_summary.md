...
5. 直行偏航主要来自底盘轮子同步/停轮可靠性，不是传感器公式错误。
6. 厂商 `stop_robot()` 只写一遍 0，现场出现过“3 个轮子停了、1 个轮子还在转”。已把本项目 `motion.stop()` 改成对 4 个电机重复写零。
...
停止时间: 1.421 s
z 轴积分: 89.603 deg
x/y 串扰: x=0.206 deg, y=-0.011 deg
```

结论：低速左转时 z 轴积分准确。

### speed=30，左转，patched stop 后
...
结论：`speed=30` 可用于闭环 90 度转向；patched stop 后停车很稳。

## 直线测试

### 污染样本，不建议用于纠偏

在 patched stop 之前，曾测到：

```text
speed=30, forward 0.8 s
z 轴积分: +36.6 deg
停车后继续偏航: +7.6 deg
```

现场观察到最后停车时有一个轮子仍在转。因此该数据主要反映 stop/轮子不同步问题，不应作为正常直行纠偏参数。
...
结论：修复 stop 后，短直行偏航显著降低。当前可按 `speed=30`、`0.8s` 约 `+2 deg` 的量级做轻微纠偏。
...
结论：电机编号和基本方向没有明显错误。落地后歪，可能来自负载、摩擦、地面接触、低速死区、I2C stop 丢包或瞬时不同步。
...
```text
move_param_forward(speed=18, param=-8)
```

会明显变差，动作期间 z 轴积分约 `+13.9 deg`。该方向不适合作为当前纠偏默认值。

后续如继续试 `move_param_forward`，应从小量开始，例如 `param=+2/-2`，并只在架空或短距离安全环境中测试。
