...
`config/shelf_manifest.json` 表达每个货架预期物品。真实场地覆盖 A 列 `A1`、`A2`、`A3`、`A4` 和 B 列 `B1`、`B2`、`B3`。需要注意，B 列从 A 列转过来后从末端开始巡检，实际经过顺序为 `B3`、`B2`、`B1`。
...
```json
{
  "patrol_order": ["A1", "A2", "A3", "A4", "B3", "B2", "B1"]
}
```
