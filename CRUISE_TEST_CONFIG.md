# 连续巡航测试配置说明

## 当前测试配置（已确认）

### ✅ 已禁用的功能
```bash
LINE_FOLLOW_ENABLED=0          # 循线逻辑完全关闭
AUTO_START_RUNTIME=0           # 启动时不自动巡逻
```

### ✅ 已启用的功能
```bash
RUN_MODE=robot                 # 真实硬件模式
SMOOTH_CRUISE_ENABLED=1        # 匀速巡航模式（待测试）
```

## 启动流程

### 1. 部署到小车
```bash
# 默认配置：不自动启动，循线禁用
./scripts/run_on_car.sh
```

### 2. 在网页手动启动巡逻
1. 打开浏览器访问 `http://192.168.1.11:5000`
2. 点击顶部的 **"开始巡逻"** 按钮
3. 小车开始匀速巡航

### 3. 手动停止
- 点击顶部的 **"立即停止"** 红色按钮（优先级最高）
- 或调用 API: `curl -X POST http://192.168.1.11:5000/api/stop`

## 验证循线已禁用

### 方法 1：检查配置
```bash
ssh pi@192.168.1.11
cd /home/pi/temp/inspection_robot
grep LINE_FOLLOW_ENABLED app.log
# 应显示：LINE_FOLLOW_ENABLED=0
```

### 方法 2：检查运行时行为
```bash
# 启动巡逻后，查看日志中是否有循线相关记录
curl http://192.168.1.11:5000/api/status | jq '.events[] | select(.type == "motion_debug") | .evidence.stage' | grep -i line

# 如果循线已禁用，应该无任何输出
# 如果看到 "line_follow_step" 或 "line_follow_auto_enter"，说明循线未正确关闭
```

### 方法 3：观察实际行为
当小车行驶到部分黑胶带（非完整边界）时：
- ✅ **正确行为（循线禁用）**: 小车继续直行，忽略部分黑胶带
- ❌ **错误行为（循线启用）**: 小车开始横向 strafe 调整位置

## 当前巡逻模式

### 边界检测逻辑（保留）
```
触发条件: 四路红外同时检测到黑色 (0,0,0,0)
动作: 停车 → 右转 90° → 继续巡航
用途: 列端转向
```

### 循线逻辑（已禁用）
```
触发条件: 部分红外检测到黑色，如 (0,1,1,1) 或 (1,0,0,1)
动作: 无（LINE_FOLLOW_ENABLED=0）
原逻辑: strafe_left/right 横向纠偏
```

### 航向保持（启用）
```
触发条件: 陀螺仪检测到偏航 > 3°
动作: 短暂 rotate_left/right 纠偏脉冲
用途: 保持直线行驶
```

## 预期行为对照表

| 场景 | LINE_FOLLOW=0 (当前) | LINE_FOLLOW=1 (已禁用) |
|------|---------------------|----------------------|
| 完整黑胶带 (0,0,0,0) | 右转 90° | 右转 90° |
| 左侧偏黑 (0,1,1,1) | 直行（忽略） | 左移纠偏 strafe_left |
| 右侧偏黑 (1,1,1,0) | 直行（忽略） | 右移纠偏 strafe_right |
| 居中白线 (1,0,0,1) | 直行（忽略） | 直行跟随 |
| 陀螺仪偏航 6° | rotate 纠偏 | rotate 纠偏（叠加循线） |

## 测试重点

### ✅ 应该测试的内容
1. 匀速巡航直线性（30秒内偏离 < 30cm）
2. 航向纠偏收敛（偏离后 2-3 秒恢复）
3. 边界转向精度（90° ± 10°）
4. 手动接管响应（转向命令完整执行）
5. 急停响应速度（< 0.5 秒）

### ❌ 不应该出现的现象
1. 遇到部分黑胶带时横向移动（说明循线未禁用）
2. 原地转圈或持续弧线（R3 回归）
3. 手动转向无响应（R1 回归）
4. 停不下来或延迟停止
5. 部分车轮停止而其他轮子继续转动

## 紧急回滚

如果测试中发现严重问题，立即执行：

```bash
# 方案 1：关闭匀速巡航，回退到经典短步巡逻
SMOOTH_CRUISE_ENABLED=0 ./scripts/run_on_car.sh

# 方案 2：完全停止巡逻
curl -X POST http://192.168.1.11:5000/api/stop
```

## 配置文件位置

- 启动脚本: `scripts/run_on_car.sh`
- 应用入口: `app.py`
- 运行时配置: `src/inspection_robot/runtime.py:128` (LINE_FOLLOW_ENABLED)
- 默认值: `_env_bool("LINE_FOLLOW_ENABLED", True)` → **已在启动脚本强制设为 0**

## 联系信息

如有疑问或发现异常，请查看：
- 审查报告: `代码审查完成后的输出`
- 测试清单: 审查报告 D 节（车上验证清单）
- 日志位置: `/home/pi/temp/inspection_robot/app.log`
