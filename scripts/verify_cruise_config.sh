...
if grep -q 'AUTO_START_RUNTIME="${AUTO_START_RUNTIME:-0}"' scripts/run_on_car.sh; then
    echo "  ✅ AUTO_START_RUNTIME 默认值为 0（不自动启动）"
else
    echo "  ⚠️  AUTO_START_RUNTIME 配置异常"
    grep "AUTO_START_RUNTIME" scripts/run_on_car.sh || true
fi
...
if ssh -o ConnectTimeout=3 "$CAR_HOST" "test -d '$CAR_DIR'" 2>/dev/null; then
    ...
    if ssh "$CAR_HOST" "test -f '$CAR_DIR/app.log'" 2>/dev/null; then
        ...
        ssh "$CAR_HOST" "cd '$CAR_DIR' && tail -50 app.log | grep -E 'LINE_FOLLOW_ENABLED|AUTO_START_RUNTIME|SMOOTH_CRUISE_ENABLED|Started on' | tail -5" 2>/dev/null || echo "     未找到配置记录"
    fi
else
...
echo "   2. AUTO_START_RUNTIME=0 (不自动启动)"
