...
RUN_MODE='$RUN_MODE' LINE_FOLLOW_ENABLED='$LINE_FOLLOW_ENABLED' SMOOTH_CRUISE_ENABLED='$SMOOTH_CRUISE_ENABLED' PORT='$PORT' FLAGS_use_mkldnn=0 OMP_NUM_THREADS=1 CPU_NUM=1 nohup \"\$PYTHON_CMD\" app.py > app.log 2>&1 & echo \$!
"
echo "Started on $CAR_HOST:$CAR_DIR with RUN_MODE=$RUN_MODE, LINE_FOLLOW_ENABLED=$LINE_FOLLOW_ENABLED, SMOOTH_CRUISE_ENABLED=$SMOOTH_CRUISE_ENABLED, PORT=$PORT"
